...
| `npm run reset-commune-draws --workspace server -- --year=2027 --yes`         | **Development tool.** The batch version: undoes every commune's pool freeze and/or draw execution at once — a dry run without `--yes`. Participants/applications are kept, only pool/result/execution state resets, so it's a much lighter reset than `reset-participants` + re-seeding |
...
| `npm run draft-commune-draws --workspace server -- --year=2027`               | **Development tool.** The inverse: returns every `READY` commune draw to `DRAFT` (optionally scoped to one year) — for walking the workflow again from the start after a reset. Leaves `LOCKED`/`COMPLETED` alone |
