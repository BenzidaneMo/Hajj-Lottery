...
function toRowDto(row: ImportRow): ImportRowDto {
  return {
    ...
    firstNameAr: neutralizeOptional(row.firstNameAr) ?? '',
    lastNameAr: neutralizeOptional(row.lastNameAr) ?? '',
    firstNameLatin: neutralizeOptional(row.firstNameLatin) ?? '',
    lastNameLatin: neutralizeOptional(row.lastNameLatin) ?? '',
